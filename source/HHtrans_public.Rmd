---
title: "Household Transmission of SARS-COV-2 in Geneva: Primary Analyses"
author: "Qifang Bi et al"
date: "10/3/2020"
...
## stan options
n_chains <- 1
n_iter <- 10 #1000 for the manuscipt
n_warmup <- 5 #250 for the manuscipt
n_treedepth <- 20
...
#dat_full <-  read.csv(here("generated_data/dat_full.csv")) 

## dummy data
dat_full <- read.csv(here("data/dummy_dat_full.csv"))
```
...
sero_dat <- dat_full %>% 
  mutate(pos=1*IgG_pos, # turn logi to numeric
         symptom_any_mod = ifelse(pos==0|debut_symptom==1|debut_symptom==0, 0, symptom_any)) %>% # symptomatic and seropositive + having symptom
  ...
  select(Household_codbar, codbar_entry, week, datededebut,
         ...
         debut_symptom) %>% 
  mutate(sex = relevel(as.factor(sex), sex_ref), 
         age_cat = relevel(as.factor(age_cat), age_ref)) 
...
```{r staninput_sensitivity_analysis}
# similar to what we did above but change seropositivity cutoff
# format the cleaned input data in a way that is ready for stan model
...
### generate stan model output for all 9 models + other sensitivity analyses
```{r model output}
...
# the rest are other models in sensitivity analysis that are presented in 
# supplemental figures, but not presented in supplemental table 2
# model 5, but change IgG seropositive cutoff to 1.5 (results shown in figure s2)
...
# model 5, but restrict data to first 6 weeks (results shown in figure s3)
hh_del <- sero_dat_expand %>% filter(is.na(reduit_famavg)) %>% pull(Household_codbar)
...
# model 5, but restrict data to last 6 weeks (results shown in figure s3)
hh_del <- sero_dat_expand %>% filter(is.na(reduit_famavg)) %>% pull(Household_codbar)
...
bq_seropos_symp_contact_1d5 <- readRDS(here("generated_data/HHtransdata/bq_seropos_renduit_famavg_sym_agecat_1d5.Rds"))
bq_seropos_symp_contact_first6 <- readRDS(here("generated_data/HHtransdata/bq_seropos_renduit_famavg_sym_agecat_first6.Rds"))
bq_seropos_symp_contact_last6 <- readRDS(here("generated_data/HHtransdata/bq_seropos_renduit_famavg_sym_agecat_last6.Rds"))
...
### Figure 3. Probability of infection
```{r fig3, fig.height = 6, fig.width = 6.5}
...
plot.dat %>% 
  ...
  geom_errorbar(aes(x=age_cat, ymax=100*(1-`97.5%`), ymin=100*(1-`2.5%`)), width=.3, position=position_dodge(width=0.8)) + 
  ...
  geom_text(data=plot.dat %>%
            ...
            aes(x=age_cat,y=100*(1-`50%`), label=avg_f), vjust=-0.3,hjust=-0.2,
            position=position_dodge(width=0.8), size=3.3,show_guide = F) +
  ...
  geom_errorbar(data=nulldata, aes(x=6, ymax=100*(1-`97.5%`), ymin=100*(1-`2.5%`), group=sex), color="darkgrey", width=.1, position=position_dodge(width=0.8)) +
  geom_text(data=nulldata %>%
            mutate(avg_f=signif(100*(1-`50%`),2)),
            aes(x=age_cat,y=100*(1-`50%`), label=avg_f), vjust=-0.3,hjust=-0.2,
            position=position_dodge(width=0.8), size=3.3,show_guide = F, color="darkgrey") +
  xlab("Age group in years")


  #6x6.5
ggsave(here("figures/fig3_prob.pdf"),width=6.5,height=6)
```

### Figure 4. OR of full model
```{r fig4, sfig.height = 4, fig.width = 9}
...
ggsave(here("figures/fig4_OR_main.pdf"),width=9,height=4)
```

### SFigure 1. frequency by household size
```{r fig2, fig.height = 6, fig.width = 4}
...
ggsave(here("figures/fig2_hhcomp.pdf"),width=4,height=6)
```


### SFigure 2. Figure representation of Table 1

```{r sfig1, fig.height = 6, fig.width = 9}
...
ggsave(here("figures/Sfig1_table1.pdf"),width=9,height=6)
...
get_seropos_hh_by_var_dat <- function(myvar){
  tmp  <- sero_dat %>% 
    mutate(symptom_any_mod = ifelse(debut_symptom==1|debut_symptom==0, 0, symptom_any))  %>%
...
l5 <- glm(pos~symptom_any_mod, data= sero_dat %>%  mutate(symptom_any_mod = ifelse(debut_symptom==1|debut_symptom==0, 0, symptom_any)), "binomial" )
...
### SFigure 3. positivity definition, cutoff only

```{r sfig3, fig.height = 5, fig.width = 9} 
...
ggsave(here("figures/sfig3_OR_cutoffonly.pdf"),width=9,height=4)
```

### SFigure 4. OR figure by weeks
...
ggsave(here("figures/sfig4_OR_weeks.pdf"),width=9,height=4)
```

### SFigure 5. Prob of infection by weeks
```{r sfig5, fig.height = 6, fig.width = 6.5}
# logit of B and Q based on sex+age model
...
ggsave(here("figures/sfig5_prob_weeks.pdf"),width=6.5,height=6)
```

### SFigure 6. Proportion of infections from outside
```{r sfig6, fig.height=3.5, fig.width=5}
...
ggsave(here("figures/sfig6_extrahhinf.pdf"),width=5,height=3.5)
...
### STable 3. attributable fraction calculation
...
get_table_out <- function(inputmodel){
  ...
  mytable <- data.frame(out=c(out_dat,out_dat2,out_dat3,out_dat4,out_dat5,sym_dat),
    ...
    group_by(hhsize) %>%
    summarise(median=quantile(out,probs=0.5),
              low=quantile(out,probs=0.025),
              up=quantile(out,probs=0.975)) %>%
