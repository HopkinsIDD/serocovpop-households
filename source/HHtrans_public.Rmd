...
dat_full_kids <- dat_full # real dat_full and dat_full_kids are not the same. 
dat_full_all <- dat_full # real dat_full and dat_full_kids are not the same.
```
...
inf_in_n_sym <- c()
for (i in 1:nrow(sero_dat_expand)){
    inf_in_n_sym <- c(inf_in_n_sym, 
                      sum(
                        (sero_dat_expand$gen[i] == (sero_dat_expand$gen +1))  & 
                          (sero_dat_expand$symptom_any_mod==1)  & 
                          (sero_dat_expand$set[i] == sero_dat_expand$set) & 
                          (sero_dat_expand$Household_codbar[i] == sero_dat_expand$Household_codbar), na.rm=T
      )
    )
}
...
avoid_in_n_sym <- c()
for (i in 1:nrow(sero_dat_expand)){
    avoid_in_n_sym <- c(avoid_in_n_sym, 
                      sum(
                        (sero_dat_expand$gen[i] > (sero_dat_expand$gen+1) )  & 
                          (sero_dat_expand$symptom_any_mod==1)  & 
                          (sero_dat_expand$set[i] == sero_dat_expand$set) & 
                          (sero_dat_expand$Household_codbar[i] == sero_dat_expand$Household_codbar)
      )
    )
}
...
get_inf_in_n_sym_age <- function(sym,age){
  dat_out <- c()
  for (i in 1:nrow(sero_dat_expand)){
    dat_out <- c(dat_out, 
                      sum(
                        (sero_dat_expand$gen[i] == (sero_dat_expand$gen +1))  & 
                          (sero_dat_expand$symptom_any_mod==sym)  & 
                          (sero_dat_expand$age_cat==age)  & 
                          (sero_dat_expand$set[i] == sero_dat_expand$set) & 
                          (sero_dat_expand$Household_codbar[i] == sero_dat_expand$Household_codbar), na.rm=T
      )
    )
...
inf_in_n_sym_0509 <- get_inf_in_n_sym_age(1,"5-9")
inf_in_n_sym_1019 <- get_inf_in_n_sym_age(1,"10-19")
inf_in_n_sym_2049 <- get_inf_in_n_sym_age(1,"20-49")
inf_in_n_sym_5064 <- get_inf_in_n_sym_age(1,"50-64")
inf_in_n_sym_65up <- get_inf_in_n_sym_age(1,"65+")


# number of symptomatically infected household members that individual "i" avoided infection from up to the generation before "i" was infected in
# by age group of the potential infectors

get_avoid_in_n_sym_age <- function(sym,age_group){
  dat_out <- c()
  for (i in 1:nrow(sero_dat_expand)){
    dat_out <- c(dat_out, 
                      sum(
                        (sero_dat_expand$gen[i] > (sero_dat_expand$gen+1) )  & 
                          (sero_dat_expand$symptom_any_mod==sym)  & 
                          (sero_dat_expand$age_cat==age_group)  & 
                          (sero_dat_expand$set[i] == sero_dat_expand$set) & 
                          (sero_dat_expand$Household_codbar[i] == sero_dat_expand$Household_codbar)
      )
    )
...
avoid_in_n_sym_0509 <- get_avoid_in_n_sym_age(1,"5-9")
avoid_in_n_sym_1019 <- get_avoid_in_n_sym_age(1,"10-19")
avoid_in_n_sym_2049 <- get_avoid_in_n_sym_age(1,"20-49")
avoid_in_n_sym_5064 <- get_avoid_in_n_sym_age(1,"50-64")
avoid_in_n_sym_65up <- get_avoid_in_n_sym_age(1,"65+")


# number of ASYMPTOMATICALLY infected household members in the immediate generation before individual "i" was infected
# but by age group of the infectors
inf_in_n_asym_0509 <- get_inf_in_n_sym_age(0,"5-9")
inf_in_n_asym_1019 <- get_inf_in_n_sym_age(0,"10-19")
inf_in_n_asym_2049 <- get_inf_in_n_sym_age(0,"20-49")
inf_in_n_asym_5064 <- get_inf_in_n_sym_age(0,"50-64")
inf_in_n_asym_65up <- get_inf_in_n_sym_age(0,"65+")
...
avoid_in_n_asym_0509 <- get_avoid_in_n_sym_age(0,"5-9")
avoid_in_n_asym_1019 <- get_avoid_in_n_sym_age(0,"10-19")
avoid_in_n_asym_2049 <- get_avoid_in_n_sym_age(0,"20-49")
avoid_in_n_asym_5064 <- get_avoid_in_n_sym_age(0,"50-64")
avoid_in_n_asym_65up <- get_avoid_in_n_sym_age(0,"65+")
...
# number of symptomatically infected household members in the immediate generation before individual "i" was infected
inf_in_n_sym <- c()
for (i in 1:nrow(sero_dat_sym_expand)){
    inf_in_n_sym <- c(inf_in_n_sym, 
                      sum(
                        (sero_dat_sym_expand$gen[i] == (sero_dat_sym_expand$gen +1))  & 
                          (sero_dat_sym_expand$symptom_any_mod==1)  & 
                          (sero_dat_sym_expand$set[i] == sero_dat_sym_expand$set) & 
                          (sero_dat_sym_expand$Household_codbar[i] == sero_dat_sym_expand$Household_codbar), na.rm=T
      )
    )
}
    ...
    avoid_in_n_sym <- c(avoid_in_n_sym, 
                      sum(
                        (sero_dat_sym_expand$gen[i] > (sero_dat_sym_expand$gen+1) )  & 
                          (sero_dat_sym_expand$symptom_any_mod==1)  & 
                          (sero_dat_sym_expand$set[i] == sero_dat_sym_expand$set) & 
                          (sero_dat_sym_expand$Household_codbar[i] == sero_dat_sym_expand$Household_codbar)
      )
    )
}
...
get_inf_in_n_sym_age <- function(sym,age){
  ...
  for (i in 1:nrow(sero_dat_sym_expand)){
    dat_out <- c(dat_out, 
                      sum(
                        (sero_dat_sym_expand$gen[i] == (sero_dat_sym_expand$gen +1))  & 
                          (sero_dat_sym_expand$symptom_any_mod==sym)  & 
                          (sero_dat_sym_expand$age_cat==age)  & 
                          (sero_dat_sym_expand$set[i] == sero_dat_sym_expand$set) & 
                          (sero_dat_sym_expand$Household_codbar[i] == sero_dat_sym_expand$Household_codbar), na.rm=T
      )
    )
  ...
  for (i in 1:nrow(sero_dat_sym_expand)){
    dat_out <- c(dat_out, 
                      sum(
                        (sero_dat_sym_expand$gen[i] > (sero_dat_sym_expand$gen+1) )  & 
                          (sero_dat_sym_expand$symptom_any_mod==sym)  & 
                          (sero_dat_sym_expand$age_cat==age_group)  & 
                          (sero_dat_sym_expand$set[i] == sero_dat_sym_expand$set) & 
                          (sero_dat_sym_expand$Household_codbar[i] == sero_dat_sym_expand$Household_codbar)
      )
    )
...
inf_in_n_sym <- c()
for (i in 1:nrow(sero_dat_expand_1d5)){
    inf_in_n_sym <- c(inf_in_n_sym, 
                      sum(
                        (sero_dat_expand_1d5$gen[i] == (sero_dat_expand_1d5$gen +1))  & 
                          (sero_dat_expand_1d5$symptom_any_mod==1)  & 
                          (sero_dat_expand_1d5$set[i] == sero_dat_expand_1d5$set) & 
                          (sero_dat_expand_1d5$Household_codbar[i] == sero_dat_expand_1d5$Household_codbar), na.rm=T
      )
    )
}
    ...
    avoid_in_n_sym <- c(avoid_in_n_sym, 
                      sum(
                        (sero_dat_expand_1d5$gen[i] > (sero_dat_expand_1d5$gen+1) )  & 
                          (sero_dat_expand_1d5$symptom_any_mod==1)  & 
                          (sero_dat_expand_1d5$set[i] == sero_dat_expand_1d5$set) & 
                          (sero_dat_expand_1d5$Household_codbar[i] == sero_dat_expand_1d5$Household_codbar)
      )
    )
}
...
# create variables related to symptom presentation
inf_in_n_sym <- c()
for (i in 1:nrow(sero_dat_expand_withkids)){
    inf_in_n_sym <- c(inf_in_n_sym, 
                      sum(
                        (sero_dat_expand_withkids$gen[i] == (sero_dat_expand_withkids$gen +1))  & 
                          (sero_dat_expand_withkids$symptom_any_mod==1)  & 
                          (sero_dat_expand_withkids$set[i] == sero_dat_expand_withkids$set) & 
                          (sero_dat_expand_withkids$Household_codbar[i] == sero_dat_expand_withkids$Household_codbar), na.rm=T
      )
    )
}
    ...
    avoid_in_n_sym <- c(avoid_in_n_sym, 
                      sum(
                        (sero_dat_expand_withkids$gen[i] > (sero_dat_expand_withkids$gen+1) )  & 
                          (sero_dat_expand_withkids$symptom_any_mod==1)  & 
                          (sero_dat_expand_withkids$set[i] == sero_dat_expand_withkids$set) & 
                          (sero_dat_expand_withkids$Household_codbar[i] == sero_dat_expand_withkids$Household_codbar)
      )
    )
}


sero_dat_expand_withkids <- sero_dat_expand_withkids %>% mutate(inf_in_n_sym = inf_in_n_sym, inf_in_n_asym = inf_in_n - inf_in_n_sym, 
                                              avoid_in_n_sym = avoid_in_n_sym, avoid_in_n_asym = avoid_in_n - avoid_in_n_sym)
...
# create variables related to symptom presentation
inf_in_n_sym <- c()
for (i in 1:nrow(sero_dat_expand_all)){
    inf_in_n_sym <- c(inf_in_n_sym, 
                      sum(
                        (sero_dat_expand_all$gen[i] == (sero_dat_expand_all$gen +1))  & 
                          (sero_dat_expand_all$symptom_any_mod==1)  & 
                          (sero_dat_expand_all$set[i] == sero_dat_expand_all$set) & 
                          (sero_dat_expand_all$Household_codbar[i] == sero_dat_expand_all$Household_codbar), na.rm=T
      )
    )
}
    ...
    avoid_in_n_sym <- c(avoid_in_n_sym, 
                      sum(
                        (sero_dat_expand_all$gen[i] > (sero_dat_expand_all$gen+1) )  & 
                          (sero_dat_expand_all$symptom_any_mod==1)  & 
                          (sero_dat_expand_all$set[i] == sero_dat_expand_all$set) & 
                          (sero_dat_expand_all$Household_codbar[i] == sero_dat_expand_all$Household_codbar)
      )
    )
}


sero_dat_expand_all <- sero_dat_expand_all %>% mutate(inf_in_n_sym = inf_in_n_sym, inf_in_n_asym = inf_in_n - inf_in_n_sym, 
                                              avoid_in_n_sym = avoid_in_n_sym, avoid_in_n_asym = avoid_in_n - avoid_in_n_sym)
...
# model 5, but change IgG seropositive cutoff to 1.5 (results shown in figure s4)

hh_del <- sero_dat_expand_1d5 %>% filter(is.na(reduit_famavg)) %>% pull(Household_codbar)
if(length(hh_del)==0) {hh_del<-0}
bq_seropos_symp_contact_1d5 <- run_analysis_bq_sym_contact(bq_model=bq_model_contact_sym,
                                                                    dat_expand=sero_dat_expand_1d5 %>% 
                                                                      mutate(age2064 = ifelse(age_cat %in% c("20-49","50-64"),1,0)) %>% 
                                                                      mutate(age65up = ifelse(age_cat=="65+",1,0)) %>% 
                                                                      filter(!Household_codbar%in%hh_del),
                                                                    coef_eqn="sex + age_cat",
                                                                    coef_eqn1="sex + age_cat + age2064:reduit_famavg +
                                                                    age65up:reduit_famavg",
                                                                    chains=n_chains,
                                                                    iter=n_iter,
                                                                    warmup=n_warmup,
...
# model 5, but restrict data to first 6 weeks (results shown in figure s5)
hh_del <- sero_dat_expand %>% filter(is.na(reduit_famavg)) %>% pull(Household_codbar)
if(length(hh_del)==0) {hh_del<-0}
bq_seropos_symp_contact_first6 <- run_analysis_bq_sym_contact(bq_model=bq_model_contact_sym,
                                                                    dat_expand=sero_dat_expand %>% 
                                                                      mutate(age2064 = ifelse(age_cat %in% c("20-49","50-64"),1,0)) %>% 
                                                                      mutate(age65up = ifelse(age_cat=="65+",1,0)) %>% 
                                                                      filter(!Household_codbar%in%hh_del) %>%
...
# model 5, but restrict data to last 6 weeks (results shown in figure s5)
hh_del <- sero_dat_expand %>% filter(is.na(reduit_famavg)) %>% pull(Household_codbar)
if(length(hh_del)==0) {hh_del<-0}
bq_seropos_symp_contact_last6 <- run_analysis_bq_sym_contact(bq_model=bq_model_contact_sym,
                                                                    dat_expand=sero_dat_expand %>% 
                                                                      mutate(age2064 = ifelse(age_cat %in% c("20-49","50-64"),1,0)) %>% 
                                                                      mutate(age65up = ifelse(age_cat=="65+",1,0)) %>% 
                                                                      filter(!Household_codbar%in%hh_del) %>%
                                                                      filter(as.character(week)>6),
                                                                    coef_eqn="sex + age_cat",
                                                                    coef_eqn1="sex + age_cat + age2064:reduit_famavg + age65up:reduit_famavg",
                                                                    chains=n_chains,
                                                                    iter=n_iter,
                                                                    warmup=n_warmup,
                                                                    control=list(max_treedepth=n_treedepth),
...
# model 5, but change definition of symptomatic cases to only those showing symptoms more than two weeks before enrollment


hh_del <- sero_dat_sym_expand %>% filter(is.na(reduit_famavg)) %>% pull(Household_codbar)
if(length(hh_del)==0) {hh_del<-0}
bq_seropos_symp_contact_2wk <- run_analysis_bq_sym_contact(bq_model=bq_model_contact_sym,
                                       dat_expand=sero_dat_sym_expand %>% 
                                         mutate(age2064 = ifelse(age_cat %in% c("20-49","50-64"),1,0)) %>% 
                                         mutate(age65up = ifelse(age_cat=="65+",1,0)) %>% 
                                         filter(Household_codbar!=hh_del),
                                       coef_eqn="sex + age_cat",
                                       coef_eqn1="sex + age_cat + 
                                       age2064:reduit_famavg + age65up:reduit_famavg",
                                       chains=n_chains,
...
# model 5, but including households only missing serostatus of 0-4 years old
hh_del <- sero_dat_expand_withkids %>% filter(is.na(reduit_famavg)) %>% pull(Household_codbar)
if(length(hh_del)==0) {hh_del<-0}
bq_seropos_symp_contact_withkids <- run_analysis_bq_sym_contact(bq_model=bq_model_contact_sym,
...
# model 5, but include all enrolled households
bq_seropos_symp_contact_all <- run_analysis_bq_sym_contact(bq_model=bq_model_contact_sym,
...
bq_seropos_symp_contact_1d5 <- readRDS(here("generated_data/HHtransdata/bq_seropos_reduit_famavg_sym_agecat_1d5.Rds"))
bq_seropos_symp_contact_first6 <- readRDS(here("generated_data/HHtransdata/bq_seropos_reduit_famavg_sym_agecat_first6.Rds"))
bq_seropos_symp_contact_last6 <- readRDS(here("generated_data/HHtransdata/bq_seropos_reduit_famavg_sym_agecat_last6.Rds"))
...
bq_seropos_symp_contact_2wk <- readRDS(here("generated_data/HHtransdata/bq_seropos_symp_contact_2wk.Rds")) 
bq_seropos_symp_contact_withkids <- readRDS(here("generated_data/HHtransdata/bq_seropos_symp_contact_withkids.Rds")) 
bq_seropos_symp_contact_all <- readRDS(here("generated_data/HHtransdata/bq_seropos_symp_contact_all.Rds")) 
```
...
plot.dat %>% 
  ...
  geom_errorbar(aes(x=age_cat, ymax=100*(1-`97.5%`), ymin=100*(1-`2.5%`)), width=.3, position=position_dodge(width=0.9)) + 
  ...
  geom_text(data=plot.dat %>%
            ...
            position=position_dodge(width=0.9), size=3.3,show_guide = F) +
  ...
  geom_errorbar(data=nulldata, aes(x=6, ymax=100*(1-`97.5%`), ymin=100*(1-`2.5%`), group=sex), color="darkgrey", width=.1, position=position_dodge(width=0.9)) +
  geom_text(data=nulldata %>%
            #mutate(avg_f=signif(100*(1-`50%`),2)),
            mutate(avg_f=formatC(100*(1-`50%`),digits=1,format="f")),
            aes(x=age_cat,y=100*(1-`50%`), label=avg_f), vjust=-0.4,hjust=-0.1,
            position=position_dodge(width=0.9), size=3.3,show_guide = F, color="darkgrey") +
...
summary(bq_seropos_symp_contact$stan_est, pars="beta_B")[[1]] %>% 
  ...
  bind_rows(data.frame(param = "3 to 5", BQ="Extra-household transmission")) %>%
  bind_rows(data.frame(param = "20-64no", BQ="Extra-household transmission")) %>%
  bind_rows(data.frame(param = "65+no", BQ="Extra-household transmission")) %>%
...
## contact
pOR_4contact <- pOR_1_data %>% filter(param %in% c("age2064:reduced_contact", "age65+:reduced_contact","65+no","20-64no")) %>% 
  mutate(age_group = case_when(param=="age2064:reduced_contact" ~ "20-64",
                               param=="20-64no" ~ "20-64",
                               param=="age65+:reduced_contact" ~ "65+",
                               param=="65+no" ~ "65+")) %>%
  mutate(param = recode(param, "age2064:reduced_contact" = "yes", "age65+:reduced_contact" = "yes", 
                        "65+no" = "no", "20-64no" = "no") ) %>%
  mutate(param = fct_relevel(param, "no", "yes")) %>%
  mutate(age_group = factor(age_group, levels= c("20-64","65+")))  %>%
  ggplot(aes(x=param, y=1/exp(`50%`),  group=age_group, lty=age_group, shape=age_group)) + 
  geom_point(position=position_dodge(width=0.8)) + 
  geom_errorbar(aes(ymax=1/exp(`97.5%`), ymin=1/exp(`2.5%`)), width=.3, position=position_dodge(width=0.8)) +
  facet_wrap(vars(BQ), nrow=2) + 
  geom_hline(yintercept = 1, linetype=2) +
  scale_y_continuous(trans=log_trans(), name = "Odds ratio (log scale)", limits=c(0.24,6),
                     breaks=c(0.25, 0.5,1,2,4,8),labels = c(0.25,0.5,1,2,4,8)) +
  xlab("reduced contacts") + theme_bw() + 
  theme(panel.grid.minor = element_blank(), panel.grid.major.y = element_blank(),legend.position=c(.7,.35), 
        axis.text=element_text(size=7), axis.title = element_text(size=8), strip.text = element_text(size = 8),
        legend.title=element_text(size=7), legend.text=element_text(size=8)) +
  scale_shape_manual(values=c(15, 17)) + labs(lty="age group",shape="age group")
...
ggarrange( ggarrange(pOR_4age + coord_flip(), pOR_4sex + coord_flip(), nrow=2,align="v", heights = c(3,2), labels=c("A","B")),
           ggarrange(pOR_4contact + coord_flip(), pOR_4sym + coord_flip(), nrow=2,align="v", heights = c(3,2),labels=c("C","D")),
           ncol=2,  widths = c(6,3), align='h'
           ) #4,9
#ggsave(here("figures/fig3OR_main.pdf"),width=9,height=4)
```
...
pOR_1_data_sens <- summary(bq_seropos_symp_contact_1d5$stan_est, pars="beta_B")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male", "5-9", "10-19", "50-64", "65+",
                 "age2064:reduced_contact", "age65+:reduced_contact")) %>%
  filter(param!="Intercept") %>%
  mutate(BQ="Extra-household transmission") %>%
  bind_rows(
    summary(bq_seropos_symp_contact_1d5$stan_est, pars="beta_Q")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male","5-9","10-19" , "50-64", "65+")) %>%
      filter(param!="Intercept") %>%
      mutate(BQ="Household transmission")
  ...
  bind_rows(
    summary(bq_seropos_symp_contact_1d5$stan_est, pars="alpha_sym")[[1]] %>% 
      as_tibble() %>% 
      mutate(param=c("sym")) %>%
      mutate(BQ="Household transmission")
  ) %>%
...
## contact
pOR_4contact_1d5 <- pOR_1_data_combined_sens2 %>% filter(param %in% c("age2064:reduced_contact", "age65+:reduced_contact","65+no","20-64no")) %>% 
  mutate(age_group = case_when(param=="age2064:reduced_contact" ~ "20-64",
                               param=="20-64no" ~ "20-64",
                               param=="age65+:reduced_contact" ~ "65+",
                               param=="65+no" ~ "65+")) %>%
  ...
  mutate(param = fct_relevel(param, "no", "yes")) %>%
  mutate(age_group = factor(age_group, levels= c("20-64","65+")))  %>%
  ggplot(aes(x=param, y=1/exp(`50%`),  group=interaction(analysis,age_group), color=analysis, lty=age_group,shape=age_group)) +
  geom_point(position=position_dodge(width=0.7)) + 
  geom_errorbar(aes(ymax=1/exp(`97.5%`), ymin=1/exp(`2.5%`)), width=.3, position=position_dodge(width=0.7)) +
  facet_wrap(vars(BQ), nrow=2) + 
  geom_hline(yintercept = 1, linetype=2) +
  ...
  theme(panel.grid.minor = element_blank(), panel.grid.major.y = element_blank(),legend.position=c(.7,.35), 
        axis.text=element_text(size=7), axis.title = element_text(size=8), strip.text = element_text(size = 8),
        legend.title=element_text(size=7), legend.text=element_text(size=8))  + 
  #scale_color_manual(labels = c("IgG ELISA\n(main analysis)", "IgG ELISA + rIFA\n(sensitivity analysis)", "IgG ELISA, cutoff 1.5 \n(sensitivity analysis)"),
  #                   values=hue_pal()(3)) +
...
ggarrange( ggarrange(pOR_4age_1d5 + coord_flip(), pOR_4sex_1d5 + coord_flip(), nrow=2,align="v", heights = c(3,2), 
                     labels=c("A","B"), legend = "none"),
           ggarrange(pOR_4contact_1d5 + coord_flip(), pOR_4sym_1d5 + coord_flip(), nrow=2,align="v", heights = c(3,2),
                     labels=c("C","D"), common.legend = T, legend = "right"),
           ncol=2,  widths = c(6,5), align='h') #4,9

#ggsave(here("figures/sfig4_OR_cutoffonly.pdf"),width=9,height=4)
```
...
pOR_1_data_first6 <- summary(bq_seropos_symp_contact_first6$stan_est, pars="beta_B")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male", "5-9", "10-19", "50-64", "65+",
                 "age2064:reduced_contact", "age65+:reduced_contact")) %>%
  filter(param!="Intercept") %>%
  mutate(BQ="Extra-household transmission") %>%
  bind_rows(
    summary(bq_seropos_symp_contact_first6$stan_est, pars="beta_Q")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male","5-9","10-19" , "50-64", "65+")) %>%
      filter(param!="Intercept") %>%
      mutate(BQ="Household transmission")
  ...
  bind_rows(
    summary(bq_seropos_symp_contact_first6$stan_est, pars="alpha_sym")[[1]] %>% 
      as_tibble() %>% 
      mutate(param=c("sym")) %>%
      mutate(BQ="Household transmission")
  ) %>%
...
pOR_1_data_last6 <- summary(bq_seropos_symp_contact_last6$stan_est, pars="beta_B")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male", "5-9", "10-19", "50-64", "65+",
                 "age2064:reduced_contact", "age65+:reduced_contact")) %>%
  filter(param!="Intercept") %>%
  mutate(BQ="Extra-household transmission") %>%
  bind_rows(
    summary(bq_seropos_symp_contact_last6$stan_est, pars="beta_Q")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male","5-9","10-19" , "50-64", "65+")) %>%
      filter(param!="Intercept") %>%
      mutate(BQ="Household transmission")
  ...
  bind_rows(
    summary(bq_seropos_symp_contact_last6$stan_est, pars="alpha_sym")[[1]] %>% 
      as_tibble() %>% 
      mutate(param=c("sym")) %>%
      mutate(BQ="Household transmission")
  ) %>%
...
pOR_4age_firstlast6 <- pOR_1_data_firstlast6 %>% filter(param %in%  c("5-9", "10-19", "20-49", "50-64", "65+")) %>%
  ...
  xlab("Age groups (years)\n") + theme_bw() + 
  theme(panel.grid.minor = element_blank(), panel.grid.major.y = element_blank(), 
        axis.text=element_text(size=7), axis.title = element_text(size=8), strip.text = element_text(size = 8)) + ylab("odds ratio")+
...
## contact
pOR_4contact_firstlast6 <- pOR_1_data_firstlast6 %>% filter(param %in% c("age2064:reduced_contact", "age65+:reduced_contact","65+no","20-64no")) %>% 
  mutate(age_group = case_when(param=="age2064:reduced_contact" ~ "20-64",
                               param=="20-64no" ~ "20-64",
                               param=="age65+:reduced_contact" ~ "65+",
                               param=="65+no" ~ "65+")) %>%
  ...
  mutate(param = fct_relevel(param, "no", "yes")) %>%
  mutate(age_group = factor(age_group, levels= c("20-64","65+")))  %>%
  ggplot(aes(x=param, y=1/exp(`50%`),  group=interaction(weeks,age_group), color=weeks, lty=age_group,shape=age_group)) +
  geom_point(position=position_dodge(width=0.7)) + 
  geom_errorbar(aes(ymax=1/exp(`97.5%`), ymin=1/exp(`2.5%`)), width=.3, position=position_dodge(width=0.7)) +
  facet_wrap(vars(BQ), nrow=2) + 
  geom_hline(yintercept = 1, linetype=2) +
  scale_y_continuous(trans=log_trans(), name = "Odds ratio (log scale)", limits=c(0.08,10),
                     breaks=c(0.125,0.25, 0.5,1,2,4,8),labels = c(0.125,0.25,0.5,1,2,4,8)) +
  xlab("reduced contacts") + theme_bw() + 
  theme(panel.grid.minor = element_blank(), panel.grid.major.y = element_blank(),legend.position=c(.7,.35), 
        axis.text=element_text(size=7), axis.title = element_text(size=8), strip.text = element_text(size = 8),
        legend.title=element_text(size=7), legend.text=element_text(size=8))  + 
  #scale_color_manual(labels = c("", "IgG ELISA + rIFA\n(sensitivity analysis)"),
  #values=hue_pal()(3)) +
...
ggarrange( ggarrange(pOR_4age_firstlast6 + coord_flip(), pOR_4sex_firstlast6 + coord_flip(), nrow=2,align="v", heights = c(3,2), 
                     labels=c("A","B"), legend = "none"),
           ggarrange(pOR_4contact_firstlast6 + coord_flip(), pOR_4sym_firstlast6 + coord_flip(), nrow=2,align="v", heights = c(3,2),
                     labels=c("C","D"), common.legend = T, legend = "right"),
           ncol=2,  widths = c(6,4), align='h') #4,9

#ggsave(here("figures/sfig5_OR_weeks.pdf"),width=9,height=4)
```
...
pOR_1_data_sens_2wk <- summary(bq_seropos_symp_contact_2wk$stan_est, pars="beta_B")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male", "5-9", "10-19", "50-64", "65+",
                 "age2064:reduced_contact", "age65+:reduced_contact")) %>%
  filter(param!="Intercept") %>%
...
pOR_1_data_combined_sens3 <- pOR_1_data %>% mutate(symptom_def="All symptomatic infections") %>% 
  bind_rows(
    pOR_1_data_sens_2wk %>% mutate(symptom_def="Only those with symptom onset\nover 2weeks from enrollment")
  )
...
## contact
pOR_4contact_2wk <- pOR_1_data_combined_sens3 %>% filter(param %in% c("age2064:reduced_contact", "age65+:reduced_contact","65+no","20-64no")) %>% 
  mutate(age_group = case_when(param=="age2064:reduced_contact" ~ "20-64",
                               param=="20-64no" ~ "20-64",
                               param=="age65+:reduced_contact" ~ "65+",
                               param=="65+no" ~ "65+")) %>%
  ...
  mutate(param = fct_relevel(param, "no", "yes")) %>%
  mutate(age_group = factor(age_group, levels= c("20-64","65+")))  %>%
  ggplot(aes(x=param, y=1/exp(`50%`),  group=interaction(symptom_def,age_group), color=symptom_def, lty=age_group,shape=age_group)) +
  geom_point(position=position_dodge(width=0.7)) + 
  geom_errorbar(aes(ymax=1/exp(`97.5%`), ymin=1/exp(`2.5%`)), width=.3, position=position_dodge(width=0.7)) +
  facet_wrap(vars(BQ), nrow=2) + 
  geom_hline(yintercept = 1, linetype=2) +
  ...
  theme(panel.grid.minor = element_blank(), panel.grid.major.y = element_blank(),legend.position=c(.7,.35), 
        ...
        legend.title=element_text(size=7), legend.text=element_text(size=8))  + 
  scale_shape_manual(values=c(15, 17)) +
  guides(col=guide_legend(title="Definition of symptomatic infections"))
...
ggarrange( ggarrange(pOR_4age_2wk + coord_flip(), pOR_4sex_2wk + coord_flip(), nrow=2,align="v", heights = c(3,2), 
                     labels=c("A","B"), legend = "none"),
           ggarrange(pOR_4contact_2wk + coord_flip(), pOR_4sym_2wk + coord_flip(), nrow=2,align="v", heights = c(3,2),
                     labels=c("C","D"), common.legend = T, legend = "right"),
           ncol=2,  widths = c(6,6), align='h') #4,9

#ggsave(here("figures/sfig6_OR_2wksym.pdf"),width=9,height=4)
```
...
bq_pars <- rstan::extract(bq_seropos_symp_contact$stan_est)
...
bq_pars_seropos_symp_contact <- extract(bq_seropos_symp_contact$stan_est)
gen <- bq_pars_seropos_symp_contact$max_gen
hhcase <- bq_seropos_symp_contact$input_data %>%  distinct(household_id, hh_inf_n) %>% pull(hh_inf_n)
...
# model 5...symptomatic infection definition
#"reduce_contact only"

#"beta-B"
summary(bq_seropos_symp_contact_2wk$stan_est, pars="beta_B")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male", "5-9", "10-19", "50-64", "65+",
                 "age2064:reduced_contact", "age65+:reduced_contact")) %>% 
  transmute(Category=param,
            `Coefficient estimate (95% CI)` = 
              paste0(formatC(1/exp(`50%`),digits=1,format="f"), " (",
                     formatC(1/exp(`97.5%`),digits=1,format="f"), ", ",
                     formatC(1/exp(`2.5%`),digits=1,format="f"), ")")) -> tbreduce_sym
  #knitr::kable(linesep="")

#"beta_Q"
summary(bq_seropos_symp_contact_2wk$stan_est, pars="beta_Q")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male","5-9","10-19" , "50-64", "65+")) %>%
  transmute(Category=param,
            `Coefficient estimate (95% CI)` = 
              paste0(formatC(1/exp(`50%`),digits=1,format="f"), " (",
                     formatC(1/exp(`97.5%`),digits=1,format="f"), ", ",
                     formatC(1/exp(`2.5%`),digits=1,format="f"), ")")) -> tqreduce_sym
  #knitr::kable(linesep="")

#"alpha_sym"
summary(bq_seropos_symp_contact_2wk$stan_est, pars="alpha_sym")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("sym")) %>%
  transmute(Category=param,
            `Coefficient estimate (95% CI)` = 
              paste0(formatC(1/exp(`50%`),digits=1,format="f"), " (",
                     formatC(1/exp(`97.5%`),digits=1,format="f"), ", ",
                     formatC(1/exp(`2.5%`),digits=1,format="f"), ")")) -> tareduce_sym
  #knitr::kable(linesep="")
...
# model 5...all households
#"reduce_contact only"

#"beta-B"
summary(bq_seropos_symp_contact_all$stan_est, pars="beta_B")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male", "5-9", "10-19", "50-64", "65+",
                 "age2064:reduced_contact", "age65+:reduced_contact")) %>% 
  transmute(Category=param,
            `Coefficient estimate (95% CI)` = 
              paste0(formatC(1/exp(`50%`),digits=1,format="f"), " (",
                     formatC(1/exp(`97.5%`),digits=1,format="f"), ", ",
                     formatC(1/exp(`2.5%`),digits=1,format="f"), ")")) -> tbreduce_all
  #knitr::kable(linesep="")

#"beta_Q"
summary(bq_seropos_symp_contact_all$stan_est, pars="beta_Q")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male","5-9","10-19" , "50-64", "65+")) %>%
  transmute(Category=param,
            `Coefficient estimate (95% CI)` = 
              paste0(formatC(1/exp(`50%`),digits=1,format="f"), " (",
                     formatC(1/exp(`97.5%`),digits=1,format="f"), ", ",
                     formatC(1/exp(`2.5%`),digits=1,format="f"), ")")) -> tqreduce_all
  #knitr::kable(linesep="")

#"alpha_sym"
summary(bq_seropos_symp_contact_all$stan_est, pars="alpha_sym")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("sym")) %>%
  transmute(Category=param,
            `Coefficient estimate (95% CI)` = 
              paste0(formatC(1/exp(`50%`),digits=1,format="f"), " (",
                     formatC(1/exp(`97.5%`),digits=1,format="f"), ", ",
                     formatC(1/exp(`2.5%`),digits=1,format="f"), ")")) -> tareduce_all
  #knitr::kable(linesep="")
...
#"beta-B"
summary(bq_seropos_symp_contact_withkids$stan_est, pars="beta_B")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male", "5-9", "10-19", "50-64", "65+",
                 "age2064:reduced_contact", "age65+:reduced_contact")) %>% 
  transmute(Category=param,
            `Coefficient estimate (95% CI)` = 
              paste0(formatC(1/exp(`50%`),digits=1,format="f"), " (",
                     formatC(1/exp(`97.5%`),digits=1,format="f"), ", ",
                     formatC(1/exp(`2.5%`),digits=1,format="f"), ")")) -> tbreduce_withkids
  #knitr::kable(linesep="")

#"beta_Q"
summary(bq_seropos_symp_contact_withkids$stan_est, pars="beta_Q")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("Intercept", "Male","5-9","10-19" , "50-64", "65+")) %>%
  transmute(Category=param,
            `Coefficient estimate (95% CI)` = 
              paste0(formatC(1/exp(`50%`),digits=1,format="f"), " (",
                     formatC(1/exp(`97.5%`),digits=1,format="f"), ", ",
                     formatC(1/exp(`2.5%`),digits=1,format="f"), ")")) -> tqreduce_withkids
  #knitr::kable(linesep="")

#"alpha_sym"
summary(bq_seropos_symp_contact_withkids$stan_est, pars="alpha_sym")[[1]] %>% 
  as_tibble() %>% 
  mutate(param=c("sym")) %>%
  transmute(Category=param,
            `Coefficient estimate (95% CI)` = 
              paste0(formatC(1/exp(`50%`),digits=1,format="f"), " (",
                     formatC(1/exp(`97.5%`),digits=1,format="f"), ", ",
                     formatC(1/exp(`2.5%`),digits=1,format="f"), ")")) -> tareduce_withkids
  #knitr::kable(linesep="")




tbreduce_withkids %>% full_join(tqreduce_withkids %>% bind_rows(tareduce_withkids), by="Category") %>% 
  full_join(tbreduce_all, by="Category") %>% full_join(tqreduce_all %>% bind_rows(tareduce_all), by="Category") %>%
  full_join(tbreduce_sym, by="Category") %>% full_join(tqreduce_sym %>% bind_rows(tareduce_sym), by="Category") %>%
  full_join(tbreduce, by="Category") %>% full_join(tqreduce %>% bind_rows(tareduce), by="Category") %>% 
  filter(Category != "Intercept") %>%
  mutate(lineid = c(5,1,2,3,4,7,8,6)) %>%
  arrange(lineid) %>% select(-lineid) %>%
  `colnames<-`(c("Category","Odds Ratio (95% CI)","Odds Ratio (95% CI)", "Odds Ratio (95% CI)", "Odds Ratio (95% CI)",
                 "Odds Ratio (95% CI)", "Odds Ratio (95% CI)")) %>% 
  kable(linksep="") %>%
