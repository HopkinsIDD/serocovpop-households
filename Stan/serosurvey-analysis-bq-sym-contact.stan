...
//
  data {
    ...
    int<lower=1> p_vars1; //number of variables to adjust for for B
    matrix[N_survey_expand, p_vars1] X1; //variable model matrix for B
    ...
    int avoid_in_n_sym [N_survey_expand]; // total number of symptomatically infected household members that individual "i" avoided infection up to the generation before "i" was infected in
    int inf_in_n_asym [N_survey_expand]; // total asymptomatically infected the generation before hh member "i" was infected
    int avoid_in_n_asym [N_survey_expand]; // total number of asymptomatically infected household members that individual "i" avoided infection from up to the generation before "i" was infected in
...
parameters {
  vector[p_vars1] beta_B; //  coefficients for community vars
